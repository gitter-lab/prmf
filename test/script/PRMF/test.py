...
X = U.dot(V.transpose())
nodelist = list(map(lambda x: str(x), range(n_genes)))
...
for k in range(k_latent):
  ...
  for i in range(len(inds)):
    G.node[i]['name'] = str(inds[i])

  manifold_fp = 'graph{}.graphml'.format(k)
  nx.write_graphml(G, manifold_fp)
  manifold_fps.append(manifold_fp)

sp.check_call(args=['nmf_pathway.py', "--data", "data.tsv", "--manifolds"] + manifold_fps + ["--nodelist", 'nodelist.txt', "--outdir", os.curdir, '--delimiter', '\t', '--seed', "1"])
